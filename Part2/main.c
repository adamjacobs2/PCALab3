...
int main (int argc, char *argv[]){
  ...
  int NRA =  atoi(argv[1]); //rows in A
  int NCA_RB = atoi(argv[2]); //rows in B = columns in B
  int NCB = atoi(argv[3]); // columns in B
    ...
    if (taskid == MASTER)
    {
        ...
        for (dest=1; dest<=numworkers; dest++)
        {
          ...
          MPI_Send(&A[offset][0], rows*NCA_RB, MPI_INT, dest, mtype,
                    MPI_COMM_WORLD);
          MPI_Send(B, NCA_RB*NCB, MPI_INT, dest, mtype, MPI_COMM_WORLD);         
          offset = offset + rows;
        }
  ...
  /**************************** worker task ************************************/
    if (taskid > MASTER)
    {
        ...
        MPI_Recv(A, rows*NCA_RB, MPI_INT, MASTER, mtype, MPI_COMM_WORLD, &status);
        MPI_Recv(B, NCA_RB*NCB, MPI_INT, MASTER, mtype, MPI_COMM_WORLD, &status);
